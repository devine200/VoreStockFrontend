...
export function TicketsSidebar({
  ...
  return (
    <aside className="flex w-full min-w-0 flex-col overflow-hidden rounded-2xl border border-[#ebebec] bg-white lg:w-[508px] lg:max-w-[508px] lg:shrink-0">
      <div className="flex items-center justify-between px-4 py-4">
        <h2 className="text-[14px] font-semibold leading-5 text-[#1a1e26]">Your tickets</h2>
        {openCount > 0 ? (
        ...
        {tickets.length === 0 ? (
          ...
          tickets.map((ticket) => (
            <button
              ...
              className={cn(
                'flex w-full min-w-0 items-start justify-between gap-3 rounded-xl px-[19px] py-[17px] text-left transition',
      ...
      <div className="border-t border-[#ebebec] px-4 py-4">
        <button
          type="button"
          onClick={onNewTicket}
          className="w-full text-center text-[13px] font-medium text-[#480516] hover:underline"
        >
          + Open a new ticket
        </button>
      </div>
